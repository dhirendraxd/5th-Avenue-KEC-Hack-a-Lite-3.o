...
export type EquipmentCategory = 'construction' | 'events' | 'manufacturing' | 'cleaning' | 'logistics';

export type RentalStatus = 'requested' | 'approved' | 'active' | 'completed' | 'declined' | 'extension_requested';
...
export const roleLabels: Record<UserRole, { label: string; description: string; color: string }> = {
  owner: { 
    ...
    description: 'Full access to all features including financials and team management',
    color: 'text-primary'
  },
  operations_manager: { 
...
export const defaultPickupChecklist: ChecklistItem[] = [
  ...
  { id: 'p2', label: 'All accessories and attachments present', checked: false },
...
const sampleReviews: Review[] = [
  {
    ...
    rating: 5,
    comment: 'Excellent condition, started right up. Owner was very helpful with operating tips.',
  ...
  },
  {
    ...
    rating: 4,
    comment: 'Good equipment, minor wear but performed as expected for our project.',
  ...
  },
  {
    ...
    rating: 5,
    comment: 'Second time renting this. Always in great shape and owner is professional.',
...
export const mockEquipment: Equipment[] = [
  {
    ...
    description: 'Heavy-duty hydraulic excavator perfect for earthmoving, trenching, and demolition projects. Well-maintained with low hours.',
    ...
    availability: {
      ...
      minRentalDays: 3,
      bufferDays: 1,
      availableRanges: [{ start: new Date('2024-01-15'), end: new Date('2024-06-30') }],
    },
    ...
    cancellationPolicy: 'Free cancellation up to 48 hours before pickup. 50% refund within 48 hours. No refund for no-shows.',
    locationId: 'loc1',
  ...
  },
  {
    ...
    description: 'Reliable 5,000 lb capacity forklift ideal for warehouse and loading dock operations. Excellent fuel efficiency.',
    category: 'logistics',
    ...
    availability: {
      ...
      minRentalDays: 1,
      bufferDays: 0,
      availableRanges: [{ start: new Date('2024-01-01'), end: new Date('2024-12-31') }],
    },
    ...
    cancellationPolicy: 'Free cancellation up to 24 hours before pickup. No refund after that.',
    locationId: 'loc1',
    locationName: 'Main Warehouse',
  },
  {
    ...
    description: 'Commercial-grade portable generator for construction sites, events, and emergency backup. Ultra-quiet operation.',
    category: 'events',
    ...
    availability: {
      blockedDates: [new Date('2024-03-01'), new Date('2024-03-02'), new Date('2024-03-03')],
      minRentalDays: 2,
      bufferDays: 1,
      availableRanges: [{ start: new Date('2024-02-01'), end: new Date('2024-12-31') }],
    },
    features: ['Auto Transfer Switch', 'Sound Attenuated', 'Digital Controls'],
    usageNotes: 'Diesel fuel not included. Delivery available for additional fee.',
    insuranceProtected: true,
    condition: 'excellent',
    totalRentals: 89,
    reviews: sampleReviews,
    cancellationPolicy: 'Free cancellation up to 72 hours before pickup. 25% fee within 72 hours.',
    locationId: 'loc2',
  ...
  },
  {
    ...
    description: 'Complete lighting and sound system for corporate events, conferences, and productions. Includes setup guide.',
    category: 'events',
    ...
    availability: {
      ...
      minRentalDays: 1,
      bufferDays: 0,
      availableRanges: [{ start: new Date('2024-01-20'), end: new Date('2024-08-15') }],
    },
    features: ['16-Channel Mixer', 'LED Par Cans', 'Wireless Microphones'],
    usageNotes: 'Basic setup assistance included. Full technician support available.',
    insuranceProtected: true,
    condition: 'good',
    totalRentals: 45,
  ...
  },
  {
    ...
    description: 'Precision vertical machining center for prototyping and small batch production. Includes tool library.',
    ...
    availability: {
      ...
      minRentalDays: 5,
      bufferDays: 2,
      availableRanges: [{ start: new Date('2024-03-01'), end: new Date('2024-09-30') }],
    },
    features: ['40-Taper Spindle', '20-Position Tool Changer', 'Probing System'],
    usageNotes: 'CNC programming experience required. Training available.',
    insuranceProtected: true,
    condition: 'excellent',
    totalRentals: 12,
    reviews: [sampleReviews[1]],
    cancellationPolicy: 'Free cancellation up to 1 week before pickup. 50% fee within 1 week.',
    locationId: 'loc3',
  ...
  },
  {
    ...
    description: 'Industrial-grade hot water pressure washer for deep cleaning, surface preparation, and maintenance.',
    category: 'cleaning',
...
export const mockRentalRequests: RentalRequest[] = [
  ...
  },
  {
    ...
    ownerNotes: 'Please coordinate pickup time the day before. Equipment will be at main warehouse.',
    pickupChecklist: defaultPickupChecklist,
    returnChecklist: defaultReturnChecklist,
  },
  {
    ...
    pickupChecklist: defaultPickupChecklist.map(item => ({ ...item, checked: true })),
    returnChecklist: defaultReturnChecklist,
    extensionRequest: {
  ...
  },
  {
    ...
    pickupChecklist: defaultPickupChecklist.map(item => ({ ...item, checked: true })),
    returnChecklist: defaultReturnChecklist.map(item => ({ ...item, checked: true })),
  },
];
...
export const conditionLabels: Record<EquipmentCondition, { label: string; color: string }> = {
  excellent: { label: 'Excellent', color: 'text-success' },
  good: { label: 'Good', color: 'text-primary' },
  fair: { label: 'Fair', color: 'text-warning' },
};

export const statusColors: Record<RentalStatus, { bg: string; text: string; label: string }> = {
  requested: { bg: 'bg-warning/10', text: 'text-warning', label: 'Pending Approval' },
  ...
  extension_requested: { bg: 'bg-accent/10', text: 'text-accent', label: 'Extension Requested' },
};

export type SortOption = 'most_rented' | 'highest_rated' | 'closest' | 'price_low' | 'price_high';
